...
export default function MacroCards() {
  ...
  const dxyPrev = dxy.length > 30 ? dxy[dxy.length - 31] : dxy[0]
  ...
  return (
    <div className="rounded-lg border border-border-card bg-bg-card p-4 h-full flex flex-col">
      ...
      <div className="grid grid-cols-1 gap-3 flex-1">
        {SERIES_CARDS.map(({ key, label, unit }) => {
        ...
        {isLoading ? (
          ...
          <div className="rounded border border-border-card bg-bg-row p-3">
          ...
          <div className="rounded border border-border-card bg-bg-row p-3">
            ...
            <div className="flex items-baseline gap-2 mt-1">
              ...
              >
                ...
                {Math.abs(dxyChange).toFixed(1)} · 30d
              </span>
            </div>
